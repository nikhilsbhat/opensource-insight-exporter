...
APP_NAME?=opensource-insight-exporter
...
test.setup.up: render.prometheus.config ## Brings up prometheus and grafana setup by running container to test the exporter.
	@docker-compose -f infrastructure/docker-compose.infra.yaml up -d

test.setup.purge: ## Teardown prometheus and grafana setup.
...
generate.document: ## generates cli documents
	@go generate github.com/nikhilsbhat/opensource-insight-exporter/docs
